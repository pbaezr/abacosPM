...
% Esta función determina la armadura dada la geometría y esfuerzos en el elemento
...
if handles.datosValidos    
    ...
    opcion = handles.uibuttongroup1.UserData; % 1, 2, 3, 4 ó 5
    ...
    if ~isempty(b) && ~isempty(h) && ~isempty(Mu) && ~isempty(Pu)        
        ...
        % si la carga axial es negativa (tracción), trazar nuevamente los diagramas para incluir la rama con P<0
        % (por defecto solo se incluye la flexo-compresión)
        ...
        % transformar Mu y Pu a variables adimensionales e incorporar al gráfico
        xu = 10^6*Mu/(fc*b*h^2);
        yu = 10^3*Pu/(fc*b*h);        
        plot(xu,yu,'o','markerfacecolor','r','tag','punto')
        
        % determinar cual es la curva más cercana al par (xu,yu)
        x = cell(8,1);
        y = cell(8,1);
        dist = zeros(1,8);
        for i = 1:8 % cuantías de referencia de 1 a 8%
        ...
        l = h; % largo de referencia para el cálculo de la cuantía de acero
        if opcion == 4, l = h*(1-gamma); end % para muros con elementos de borde
        
        % determinar la cuantía y área requerida de acero para las dimensiones y esfuerzos especificados
        % valor que indica si el par (xu,yu) está bajo la curva definida por ind1
        ...
        elseif ind1 == 8 && ~flag % no calcular ni graficar si rho es mayor a 8%
            ...
            elseif inpolygon(xu,yu,[0 lim_phi_065.XData xmax xmax 0],...
                ...
                caso = 2; % zona de transición (0.65 < phi < 0.9)
                phi = 0.9:-0.005:0.65;
            else
                caso = 3; % control por compresión
                phi = 0.65;
            end            
            ...
            if caso ~=3 || handles.radiobutton1.Value == 0 || ~inpolygon(xu,yu,[0 handles.xlim_08Pmax 0],...
                ...
                % para phi = 0.65 la función 'interaccionPM2' puede requerir 
                % un mayor número de cargas de muestra para arrojar resultados
                if caso == 3, Pitera = linspace(0,Pu/phi,10); end
                
                ind = length(Pitera);
                error = tol+1;
                rho = 0.01*ind1; % se itera a partir de la cuantía asociada a la curva más cercana al par (xu,yu)
                dAs = 0; % diferencial de área de acero para el inicio de la iteración
                
                % se usa el mismo error definido para el análisis seccional
                % (aunque aquel esté asociado a la sumatoria de cargas axiales)
                while abs(error) > tol
                    rho = rho+dAs/(b*l);
                    [As,ys] = distribuirAs(b,h,opcion,gamma,rho,rho_w);
                    [M,P,phi_red] = interaccionPM2(b,h,As,ys,parametros,n,tol,false,Pitera);
                    
                    % determinar el índice para el cual Pu = phi*P
                    ...
                    % función 'interaccionPM2') probar con un vector de cargas más "tupido" y reiniciar las iteraciones
                    if ind > length(M)
                    ...
                    % calcular el error asociado al momento y el diferencial de área para la nueva iteración
                    error = Mu-phi_red(ind)*M(ind); % error en kN*m
                    dAs = 10^6*error/(fy*0.9*h); % diferencial de área en mm^2
                end
                
                % calcular y trazar el diagrama para la cuantía y As previamente determinadas
                ...
                % si el elemento es una columna, identificar el límite superior de
                % compresión por concepto de la excentricidad accidental
