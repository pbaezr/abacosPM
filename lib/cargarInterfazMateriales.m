function cargarInterfazMateriales(handles,path)
% Esta funcion crea la interfaz para ver y editar las leyes constitutivas
% del acero y el hormigon, con las cuales se calculan los diagramas de
% interaccion PM.
...
% crear popup que servira como una ventana anidada a la figura base
...
% crear un panel con pestanas para cada material (acero y hormigon)
tabgp = javax.swing.JTabbedPane;
popup.add(tabgp);

%% acero
% crear panel que sera el contenedor para la pestana asociada a la curva del acero
...
handles.modelosAcero(1).setSelected(true); % dejar el modelo elastoplastico como opcion por defecto
...
% crear un sub-sub-panel para aglutinar los inputs asociados al modelo elastoplastico
...
% crear grafico para mostrar la curva del modelo actualmente seleccionado
% (por defecto se establece el modelo elastoplastico con ey = 420/200000 = 0.0021)
...
%% hormigon
% crear panel que sera el contenedor para la pestana asociada a la curva del hormigon
...
% crear opciones para definir la ley constitutiva del hormigon a compresion
...
handles.modelosHormigon(1).setSelected(true); % dejar el modelo elastoplastico como opcion por defecto
...
% crear opciones para definir la ley constitutiva del hormigon a traccion
grupo2Tab2 = javax.swing.ButtonGroup;
radiopanel2Tab2 = javax.swing.JPanel;
radiopanel2Tab2.setLayout(java.awt.GridLayout(2,1));

opcion = {'<html>Sin resistencia <br>a traccion' '<html>Lineal hasta <br>la rotura'};
for i = 1:2
...
handles.modelosHormigonTrac(1).setSelected(true); % por defecto no considerar resistencia a traccion en el hormigon
handles.opcionesHormigonTrac = grupo2Tab2;

% dejar el modelo sin resistencia a traccion como opcion por defecto
...
% crear un sub-sub-panel para aglutinar los inputs asociados a los modelos del hormigon
...
% funcion que se ejecuta al presionar el boton 'Leyes Constitutivas'
...
if boton.isSelected
    ...
    verLeyesConstitutivas(handles) % mostrar los parametros actualmente vigentes
else
    popup.setVisible(false)
end
end

% funcion que se ejecuta cuando los inputs pierden el foco
function recalcularPostEdicion(textfield,evt,handles,popup,nTab)
evaluarInput(textfield);
elemento = evt.getOppositeComponent;

% no recalcular si se cambia de modelo, pues esto se hara directamente
% a traves del 'ActionPerformedCallback' de los radiobuttons
...
% funcion que se ejecuta despues de presionar las teclas enter o tab mientras se edita un input
...
% funcion que se ejecuta cuando un input captura el foco
function seleccionarEditText(textfield)
global altPressed;
if ~altPressed, textfield.selectAll; end
end

% funcion que verifica que el valor ingresado es los textfields son validos
...
% funcion que grafica la curva esfuerzo-deformacion del acero dependiendo 
% del modelo y los parametros establecidos
...
% graficar la ley constitutiva del acero segun la opcion y los inputs elegidos
if flag % modelo elastoplastico
...
% funcion que grafica la curva esfuerzo-deformacion del hormigon
% dependiendo del modelo y los parametros establecidos
...
% graficar la ley constitutiva del hormigon segun la opcion y los inputs elegidos
...
if ~isnan(e0)
    ...
    if flag
        ...
        if ~isnan(eu)
            if opcion == 2 % modelo de Hognestad
                ...
                if opcion == 3 % calibrado segun Collins y Porasz
                    r = 0.8+fc/17;
                    k = [ones(1,50),(0.67+fc/62)*ones(1,50)];
                else%if opcionHormigon == 4 % calibrado segun Carreira y Kuang-Han
                    r = 1.55+(fc/32.4)^3;
                    k = 1;
                end
  ...
  % traccion en el hotmigon? (incide poco y nada en el calculo)
...
% funcion que permite visualizar los ultimos modelos establecidos
% (los que fueron guardados a traves del boton 'Guardar')
function verLeyesConstitutivas(handles)
parametros = getappdata(handles.figure1,'parametrosMateriales');
inputs = handles.inputsMateriales;

% mostrar los parametros actuales de la curva esfuerzo-deformacion del acero
modeloAcero = parametros.modeloAcero;
handles.modelosAcero(modeloAcero{1}).setSelected(true);
if modeloAcero{1} == 1 % modelo elastoplastico
...
% mostrar los parametros actuales de la curva esfuerzo-deformacion del hormigon
...
% funcion que se ejecuta al presionar el boton 'Val. Predeterm.'
...
% funcion que se ejecuta cuando el boton 'Guardar' es presionado
...
% definir los parametros del acero segun el modelo utilizado
...
% definir los parametros del hormigon
...
% almacenar los nuevos parametros definidos
...
% recalcular el diagrama PM con los nuevos parametros para las leyes constitutivas de los materiales
